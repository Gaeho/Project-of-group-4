...
public class SrComMypageController {
	 ...
	 * // 북마크회원
	 * 
	 * @GetMapping(value = "ComMarkMember") public String ComMarkMember( Model
	 * model, String currentPage, CommCompany CommCompany , Bookmark bookmark) {
	 * 
	 * //------------------------------------------------------------------------
	 * 
	 ...
	 * ---------------------------------------------------------------------------
	 * 
	 * // 북마크 유무 String com_id ="tlstprp5184"; String user_id = "dmdtla054";
	 * 
	...
	public String bookmarkList(String currentPage, MemBmark memBmark, Model model ) {
		...
		System.out.println("--------------------------------------------------");
		System.out.println("SrComMypageController Start...");
		int total = scms.total();
		System.out.println("bookmarkList total->"+total);
		System.out.println("--------------------------------------------------");
		
		// Paging
		Paging pg = new Paging(total, currentPage);
		memBmark.setStart(pg.getStart());
		memBmark.setEnd(pg.getEnd());
		
		System.out.println("SrComMypageController bookmarkList Start");
		List<MemBmark> listBmark = scms.listBmark(memBmark);
		
		System.out.println("SrComMypageController listBmark.size()->"+listBmark.size());
		System.out.println("---------------------------------------------------------------");
		for(MemBmark memBmark2 : listBmark) {
			System.out.println("---------bookmarkList Start -------------");
			System.out.println("memBmark2.getCom_id()-> "+memBmark2.getCom_id());
			System.out.println("memBmark2.getUser_id()->"+memBmark2.getUser_id());
			System.out.println("memBmark2.getUser_img()->"+memBmark2.getUser_img());
			System.out.println("memBmark2.getMrk_date()-> "+memBmark2.getMrk_date());
			System.out.println("----------bookmarkList End------------------");
			
		}
		
		int itbookmark = scms.bookmarkgetinfo(memBmark);
		System.out.println("Controller itbookmark->"+itbookmark);
		model.addAttribute("itbookmark", itbookmark);
	...
	public String detail2(String user_id, CommMemResume commMemResume, Model model) {
		System.out.println("SrComMypageController detail Start...");
		CommMemResume memresume = scms.memresume(user_id);
		model.addAttribute("memresume", memresume);
		System.out.println("memresume.getUser_name()->" + memresume.getUniv_name());
		System.out.println("memresume.getUser_tel()->" + memresume.getRes_email());
		System.out.println("memresume.getUser_addr()->" + memresume.getCov_let());
	...
	public String bookmark(String com_id, String user_id, int it_bookmark, Model model) {
		...
		if (it_bookmark == 0) { // 스크랩하기
			scms.bookmarkplus(bookmark);
			imgSrc = "북마크취소IMGSRC";
		} else if (it_bookmark == 1) { // 스크랩취소
			scms.bookmarkcancle(bookmark);
			imgSrc = "북마크하기IMGSRC";
		}
